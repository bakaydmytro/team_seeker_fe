...
export default function EditProfile() {
  ...
  const onFileChange = async (e) => {
    ...
    try {
      ...
      .then(response => 
        {
          if(response.message == "Invalid file type. Only JPG, PNG are allowed.") {
            setAlertMessage(response.message)
            setCloseAlert(true)
          }
          if(response.message == "File too large") {
            setAlertMessage(response.message + " upload a photo up to 2 MB in size")
            setCloseAlert(true)
          }
      })

      const getAvatar = await getUserData()
      .then(response => {
  ...
  return (
    <div className="container profile-container">
      {closeAlert && <AlertMessage alertMessage={alertMessage} onCloseAlert={setCloseAlert}/>}
        ...
        <aside className="right-side-profile-block">
          <div className="form-block">
            ...
            <form className="change-profile-form">
              <div className="change-profile-form_block">
                <input
                  name="email"
                  placeholder="Email"
                  type="text"
